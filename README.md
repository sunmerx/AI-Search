...
npm run crawl          # 🕷️ 抓取真实数据
npm run dev            # 🖥️ http://localhost:3000
npm test               # ✅ 运行测试
```
...
curl .../api/v1/category/ai-models.json # 🏷️ 按分类拉取
```
...
| `lib/` | ⚙️ 核心逻辑 — 类型定义、筛选排序、i18n、趋势计算、周报生成 |
| `scripts/` | 🕷️ 构建脚本 — 抓取编排、源适配器、API 生成、OG 图片生成 |
...
| `worker/` | ☁️ Cloudflare Worker — AI 问答代理，隐藏 Key + IP 限流 |
| `public/` | 🖼️ 静态资源 — OG 图片、SW、SKILL.md、安装脚本 |
